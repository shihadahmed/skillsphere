...
export default function CourseGrid() {
  ...
  return (
    <section
      id="courses"
      className="py-12 bg-white text-slate-900 w-full max-w-7xl mx-auto px-4"
    >
        ...
        {categories.map((category) => {
          ...
          return (
            ...
            >
              {category}
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? "bg-blue-500 text-white" : "bg-slate-200 text-slate-500"}`}
              >
        ...
        {filteredCourses.map((course) => (
          ...
          >
            ...
            <div className="p-4 flex flex-col flex-grow">
              ...
              <button
                onClick={() => handleSeeDetails(course.id)}
                className="w-full py-2.5 border border-slate-200 hover:border-blue-600 hover:bg-blue-50 text-slate-700 hover:text-blue-600 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2 cursor-pointer"
              >
                👁️ See Details
              </button>
            </div>
